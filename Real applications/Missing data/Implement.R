...
# =============================================================================
# LUCAS COUNTY, OH HOUSE PRICE ANALYSIS-missing data
# =============================================================================
...
# create W
# Convert neighbors list to list weights, then to sparse matrix format
hlw <- nb2listw(LO_nb)          # Convert neighbors to list weights
hsn <- listw2sn(hlw)            # Convert to spatial neighbors format
W <- as(hlw, "CsparseMatrix")   # Convert to compressed sparse matrix

# -----------------------------------------------------------
# 2. Extract 1998 data
# -----------------------------------------------------------

indeces.1998 <- data.frame(hmm0)$syear1998   # Indicator for 1998 sales
indeces.1998 <- ifelse(indeces.1998 == 1, TRUE, FALSE)

# Restrict W and X to only 1998 observations
W_1998 <- W[indeces.1998, indeces.1998]
X_1998 <- X[indeces.1998, ]

# Remove redundant year dummies (syear1994–syear1998)
columns_to_remove <- c("syear1994", "syear1995", "syear1996",
                       "syear1997", "syear1998")
X_1998 <- X_1998[, !colnames(X_1998) %in% columns_to_remove]

# Response variable: log-transformed price (scaled by 100,000)
hprice_1998     <- (house$price)[indeces.1998]
loghprice_1998  <- log(hprice_1998 / 100000)

# Quick diagnostic: density plot of log-prices
plot(density(loghprice_1998))


## Final SEM inputs for 1998

# Covariates: standardized (mean 0, variance 1), excluding intercept
x <- as.matrix(X_1998[, -1]) 
x<-as.matrix(as.data.frame(scale(x))) # covariates of SEM
xm<-as.matrix(x[,1]) # Covariate of missing data model

# Response and weights matrix
y <- loghprice_1998
w <- W_1998
...
psi=c(0.1,rep(0.1,1),-0.1) # True parameter values of missing data model
...
lines(density2, col = "green", lwd = 2,lty=1)
legend("topright", legend = c("yu", "yo"), col = c("red", "green"), lwd = 2)
...
colnames(SEM.Gau.theta.sample)<-c("intercept",colnames(x),"s2","rho",
                                  "psi1","psi2","psi3")
...
colnames(YJ.SEM.Gau.theta.sample)<-c("intercept",colnames(x),"s2","rho","gamma",
                                     "psi1","psi2","psi3")
...
colnames(SEM.t.n.info.theta.sample)<-c("intercept",colnames(x),"s2","rho","df",
                                       "psi1","psi2","psi3")

head(SEM.t.n.info.theta.sample)
round(apply(SEM.t.n.info.theta.sample, 2, mean),4)
SEM.t.theta.sample<-SEM.t.n.info.theta.sample
#YJ-SEM-t
...
colnames(YJ.SEM.t.n.info.theta.sample)<-c("intercept",colnames(x),"sigma2","rho","nu","gama",rep("psi",3))
head(YJ.SEM.t.n.info.theta.sample)
round(apply(YJ.SEM.t.n.info.theta.sample, 2, mean),4)
YJ.SEM.t.theta.sample<-YJ.SEM.t.n.info.theta.sample
...
p.B0<-ggplot(data, aes(x = value, color = group)) +
  ...
  labs(title = "intercept", x =  "", y = "Density") +  # Single heading
...
SEM.t.theta.i.sample<-SEM.t.theta.sample[,ncov+4]
YJ.SEM.t.theta.i.sample<-YJ.SEM.t.theta.sample[,ncov+5]
...
p<-ncol(x)+1
names.betas<-c("(Intercept)","age","I(age^2)","I(age^3)","log(lotsize)","rooms" ,"log(TLA)","beds",
               "s2","rho",
               "psi1","psi2","psi3")
# names.psi<-c("psi1","psi2","psi3")

colnames(SEM.Gau.theta.sample)<-names.betas
...
names.betas<-c("(Intercept)","age","I(age^2)","I(age^3)","log(lotsize)","rooms" ,"log(TLA)","beds",
               "s2","rho","gamma",
               "psi1","psi2","psi3")
colnames(YJ.SEM.Gau.theta.sample)<-names.betas
...
colnames(SEM.t.n.info.theta.sample)<-c("(Intercept)","age","I(age^2)","I(age^3)","log(lotsize)","rooms" ,"log(TLA)","beds",
                                       "s2","rho","df",
                                       "psi1","psi2","psi3")
...
colnames(YJ.SEM.t.n.info.theta.sample)<-c("(Intercept)","age","I(age^2)","I(age^3)","log(lotsize)","rooms" ,"log(TLA)","beds",
                                   "s2","rho","df","gamma",
                                   "psi1","psi2","psi3")
