...
# =============================================================================
# LUCAS COUNTY, OH HOUSE PRICE ANALYSIS-Full data
# =============================================================================
...
error_model <- errorsarlm(y~x, listw = mat2listw(w),
                           zero.policy = TRUE,method = "LU")
...
colnames(SEM.Gau.theta.sample)<-c("intercept",colnames(x),"sigma2","rho")
...
colnames(YJ.SEM.Gau.theta.sample)<-c("intercept",colnames(x),"sigma2","rho","gamma")
...
colnames(SEM.t.theta.sample)<-c("intercept",colnames(x),"sigma2","rho","nu")
...
colnames(YJ.SEM.t.theta.sample)<-c("intercept",colnames(x),"sigma2","rho","nu","gama")
...
ggplot(data, aes(x = value, color = group)) +
  geom_density(alpha = 0,size=2) +  # Remove fill, keep line color
  facet_wrap(~ group, scales = "free", ncol = 2) +  # 2x2 panel layout
  labs(title = "age", x =  "", y = "Density") +  # Single heading
  scale_color_manual(values = custom_colors, labels = custom_labels) +  # Set custom colors and labels for each group
  theme_minimal() +  # Clean theme
  theme(
